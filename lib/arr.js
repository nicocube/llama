...
module.exports = (function () {
    ...
    ;['map', 'filter', 'slice'].forEach(function(k) {
        arr.prototype[k] = buildWorkable(k)
    })
    ;['map', 'filter'].forEach(function(k) {
        wrk.prototype[k] = buildChainable(k)
    })
    
    wrk.prototype.slice = function(b, e) {
        var f = {
            _arr_type: 'slice',
            b: b,
            e: e
        }
        console.log('slice:create', f)
        this.fs.push(f);
        return this
    }

    wrk.prototype.get = function() {
        var _ = this._, fs = this.fs, i = 0, h = 0, l = _.length, m = fs.length, r = []
        elem:
        for (; i < l ; i+=1) {
            var $ = _[i], j = 0;
            for (; j < m ; j+=1) {
                ...
                switch(f._arr_type){
                    ...
                    case 'slice':
                        ...
                        if (h < f.b) {
                            ...
                            if (h === f.b) {
                                h = 0
                                if (typeof f.e !== 'undefined') f.e = f.e - f.b - 1
                                f.b = 0
                                console.log('modif f:', f)
                            }
                            console.log(typeof f.e !== 'undefined' && h > f.e)
