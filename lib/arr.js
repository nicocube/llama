...
module.exports = (function () {
    'use strict'
    function arr(_) { if (! (this instanceof arr)) return new arr(_); this._ = _ }
    
    function buildWorkable(T) { return function(f, t) { console.log('work', T, this instanceof arr); var w = wrk(this._); return w[T](f, t) } }
    
    arr.prototype.map = buildWorkable('map')
    arr.prototype.filter = buildWorkable('filter')
    
    
    function wrk(_) {if (! (this instanceof wrk)) return new wrk(_); this._ = _; this.fs = [] }
    
    function buildChainable(T) { return function(f, t) { console.log('chain', T, this instanceof wrk); f = arguments.length > 1 ? f.bind(t): f; f._arr_type = T; this.fs.push(f); return this } }
    
    wrk.prototype.map = buildChainable('map')
    wrk.prototype.filter = buildChainable('filter')
    
    wrk.prototype.get = function() {
