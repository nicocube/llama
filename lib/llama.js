...
module.exports = function (mlf) {
    ...
    var formatter = {
        ...
        open_block: function (tagName, attr) {
			return '<' + tagName + (typeof attr !== 'undefined' ? this.tag_attr(attr) : '') + '>'
		},
        ...
        close_block: function (tagName) {
			return '</' + tagName + '>'
		},
        children : function (elements) { 
            return mlf(elements).map(function(e) {
            ...
            return function() {
                ...
                formatter.children(
                    mlf(arr).map(function(e) {
                        ...
                        c[k] = e
                        that._.push(c) 
                        console.log('k: "'+k+'" e: '+e, v)
                        var r = mlf(v).map(parse).get()
                        that._.pop() 
                        return r
                    })
                    .reduce(function (p,c) {
