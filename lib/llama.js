...
module.exports = function (options) {
	require('knit').inject(function (knit, wool, mime) {
		return knit.config(function (bind) {
			bind('mu').to(require('mu2'))
		...
		}).inject(function(http, fs, filter, dispatch, static, http_status, template) {
			...
			return http.createServer(function (req,res) {
				...
				dispatch.chain([
					...
					dispatch.rule({method: static_methods, url: template_regex}, template.build(options.template_folder || options.folder, options.data_folder ||  options.folder, template_regex, option.str_to_obj_inject)),
