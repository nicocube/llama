...
module.exports = function (mlf) {
	...
	LlamaArrayTag.prototype.render = function(options) {
		...
		return mlf(this.a.v).map(function(v,i) {
			...
			if ('setDelta' in options) options.setDelta(i*this.c.length)
	...
	LlamaTag.prototype.parseOptions = function (options) {
		if ('indent' in options) {
			...
			if (!('delta' in options)) options.delta = 0
			if (!('ctx' in options)) options.ctx = [{p: 0, c:false}]
			if (!('tab' in options)) options.tab = function(d) {
				var i = 0, l = d*options.indent, t =''
				for (; i < l ; i+=1) t+=' '
				return t
			}
			if (!('aroundStartTag' in options)) options.aroundStartTag = function(r) {
			...
			if (!('aroundChildrenRender' in options)) options.aroundChildrenRender = function(r) {
				return r
			}
			if (!('aroundStopTag' in options)) options.aroundStopTag = function(r) {
				this.depth-=1
				this.ctx.shift()
			...
			if (!('setDelta' in options)) options.setDelta = function(i) {
				this.delta = i
			}
			if (!('setPosition' in options)) options.setPosition = function(i) {
				this.ctx[0].p = i + this.delta
			}
			if (!('getPosition' in options)) options.getPosition = function() {
				if (this.ctx.length === 1) return -1
				return this.ctx[0].p
			}
			if (!('pushCtx' in options)) options.pushCtx = function() {
				if (this.ctx.length > 1) this.ctx[1].c = true
				this.ctx.unshift({p: 0, c:false})
			}
			if (!('hasTagChildren' in options)) options.hasTagChildren = function() {
				return this.ctx[0].c
			}
		}
	}
	LlamaTag.prototype.render = function (options) {
		...
		var r = ''
		if ('aroundStartTag' in options) r += options.aroundStartTag(this.startTag())
		else r += this.startTag()
		if (this.c.length > 0) {
			if ('aroundChildrenRender' in options) r += options.aroundChildrenRender(this.renderChildren(options))
			else r += this.renderChildren(options)
			if ('aroundStopTag' in options) r += options.aroundStopTag(this.closeTag())
			else r += this.closeTag()
		}
		return r
	}
	...
	exp.LlamaArrayVal = LlamaArrayVal
	exp.LlamaArrayTag = LlamaArrayTag
    return exp
}
