	...
	require('knit').inject(function(http, fs, wool, filter, dispatch, static, http_status, template, knit) {
		...
		var static_methods = ['HEAD', 'GET'],
			...
			dispatcher = dispatch.chain([
				...
				dispatch.rule({method: static_methods, url: template_regex}, template.build(options.template_folder || options.folder, options.data_folder ||  options.folder, template_regex, 'str_to_obj_inject' in options ? options.str_to_obj_inject(knit): undefined)),
